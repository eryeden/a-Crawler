...
def registerAnime(anime_infos, dbcur):
    ...
    if (is_registered == True):
        ...
        # dbcur.execute("")
        dbcur.execute(
            "UPDATE anime set (anime_name, anime_description, date, episode, src_link) = ((%s), (%s), (%s), (%s), (%s)) where src_link = (%s)",
            [anime_name, anime_discription, date_time, anime_episode, src_link, src_link]);


    else:
        # 登録済みでない場合
        dbcur.execute(
            "INSERT INTO anime(anime_name, anime_description, date, episode, src_link) VALUES((%s), (%s), (%s), (%s), (%s))",
            [anime_name, anime_discription, date_time, anime_episode, src_link]);
...
for itm in items:
    ...
    anime_name = itm.find("dc:subject").string
    anime_description = itm.find("description").string.replace(anime_name + '　第','')
    episode = anime_description.split("話")[0]
    ...
    print(mp4links);
    print(dt.now().strftime('%Y-%m-%d %H:%M:%S'))
    registerAnime([anime_name, anime_description, episode, src_link, mp4links, dt.now().strftime('%Y-%m-%d %H:%M:%S')],
                  cur);
